...
def load_assets():
    """Loads ML assets only when needed, keeping server startup instant."""
    global model, df
    if model is None or df is None:
        print("INFO: Lazy loading ML assets into memory...")
        try:
            model = pickle.load(open("model.pkl", "rb"))
            df = pd.read_csv("2. Machine Learning Model/epl_final.csv")
            print("INFO: ML assets loaded successfully.")
...
TEAM_NAME_MAP = {
    ...
    "Wolverhampton Wanderers FC": "Wolves", "Burnley FC": "Burnley",
    ...
    "West Ham United FC": "West Ham", "AFC Bournemouth": "Bournemouth",
    "Luton Town FC": "Luton", "Sheffield United FC": "Sheffield United",
    "Leeds United FC": "Leeds", "Sunderland AFC": "Sunderland"
}

@app.post("/predict")
def predict(request: MatchRequest):
    # Trigger the lazy loader right here before running predictions
    load_assets()

    # Normalize names using the Map
